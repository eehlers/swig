...
//	#endif
    ...
    //SimpleLibAddin::registerEnumerations();
}
...
DLLEXPORT XLOPER *xlAddInManagerInfo(XLOPER *xlAction) {
    ...
    if (1 == xlReturn.val.w) {
        ObjectHandler::scalarToOper(std::string("SimpleLibAddin " /*SLADDIN_VERSION*/), xlLongName);
    } else {
...
//#endif
        // Initialize SimpleLib functions
        ...
        err << "Error loading SimpleLibXL: " << e.what();
...
//#endif
        ...
        // Unregister SimpleLib functions
        unregisterFunctions(xDll);

        // Deallocate the Enumeration Registry
        //SimpleLibAddin::unregisterEnumerations();
...
//#endif
        ...
        Excel(xlFree, 0, 1, &xDll);
        std::ostringstream err;
        err << "Error unloading SimpleLibXL: " << e.what();
        Excel(xlcAlert, 0, 1, TempStrStl(err.str()));
        return 0;
